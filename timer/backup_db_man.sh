...
backup()
{
   ...
   mysqldump -S $sock -u$user -p$password ${bak_db} | gzip > ${bak_db}_${curDate}_man.sql.gz
   log "mysql备份文件: ${bak_db}_${curDate}_man.sql.gz 生成成功"
}
